<?php if ( post_password_required() ) : ?>
	<p><?php _e( 'This post is password protected. Enter the password to view any comments.', 'enigma-parallax' ); ?></p>
	<?php return; endif; ?>
    <?php if ( have_comments() ) : ?>
	<div class="enigma_comment_section">		
	<div class="enigma_comment_title"><h3><i class="fa fa-comments"></i><?php echo comments_number(__('No Comments','enigma-parallax'), __('1 Comment','enigma-parallax'), '% Comments'); ?></h3></div>
	<?php wp_list_comments( array( 'callback' => 'enigma_parallax_comment' ) ); ?>		
	<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
		<nav id="comment-nav-below">
			<h1 class="assistive-text"><?php _e( 'Comment navigation', 'enigma-parallax' ); ?></h1>
...
<?php if ( comments_open() ) : ?>
	...
	add_filter('wl_comment_form_default_fields','my_fields');
		...
		'logged_in_as' => '<p class="logged-in-as">' . __( "Logged in as ",'enigma-parallax' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="Log out of this account">'.__(" Log out?",'enigma-parallax').'</a>' . '</p>',
