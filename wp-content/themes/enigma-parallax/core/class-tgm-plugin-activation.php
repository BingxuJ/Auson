	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'oops'                            => __( 'Something went wrong.', 'enigma-parallax' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'enigma-parallax' ),
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'enigma-parallax' ),
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'enigma-parallax' ),
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'enigma-parallax' ),
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'enigma-parallax' ),
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'enigma-parallax' ),
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'enigma-parallax' ),
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'enigma-parallax' ),
				//'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'enigma-parallax' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'enigma-parallax' ),
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_groups ) {
					...
					$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'enigma-parallax' ), $imploded, $count ) . '</p>';
				}
				...
				$show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'enigma-parallax' ) . '</a>'  : '';
	...
	class TGMPA_List_Table extends WP_List_Table {
			...
			foreach ( $this->tgmpa->plugins as $plugin ) {
				...
				if ( ! empty( $plugin['source'] ) ) {
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
						...
						$table_data[ $i ]['source'] = __( 'Private Repository', 'enigma-parallax' );
					}
					else {
						// The plugin is pre-packaged with the theme.
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'enigma-parallax' );
					}
				}
				// The plugin is from the WordPress repository.
				else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'enigma-parallax' );
				}

				$table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'enigma-parallax' ) : __( 'Recommended', 'enigma-parallax' );

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'enigma-parallax' ) );
				}
				elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'enigma-parallax' ) );
				}
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				...
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'enigma-parallax' ) . '</a>',
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				...
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'enigma-parallax' ) . '</a>',
		...
		public function column_cb( $item ) {
			...
			if ( __( 'Private Repository', 'enigma-parallax' ) === $item['source'] ) {
				// Escape external URLs
				$plugin_url = esc_url( $plugin_url );
			}
			elseif ( __( 'Pre-Packaged', 'enigma-parallax' ) === $item['source'] ) {
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'enigma-parallax' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'status' => __( 'Status', 'enigma-parallax' ),
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'enigma-parallax' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'enigma-parallax' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( empty( $plugin_installs ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'enigma-parallax' ), '</p></div>';
					return false;
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'enigma-parallax' ) ), wp_kses_post( $imploded ) );
				}
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function install_strings() {
						...
						$this->strings['no_package']          = __( 'Install package not available.', 'enigma-parallax' );
						$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'enigma-parallax' );
						$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'enigma-parallax' );
						$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'enigma-parallax' );
						$this->strings['process_failed']      = __( 'Plugin install failed.', 'enigma-parallax' );
						$this->strings['process_success']     = __( 'Plugin installed successfully.', 'enigma-parallax' );
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						if ( $this->tgmpa->is_automatic ) {
							...
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'enigma-parallax' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'enigma-parallax' ) . '</span><span class="hidden">' . __( 'Hide Details', 'enigma-parallax' ) . '</span>.</a>';
						...
						// Default installation strings.
							...
							$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'enigma-parallax' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'enigma-parallax' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'enigma-parallax' ) . '</span>.</a>';
					...
					public function bulk_footer() {
						...
						if ( empty( $complete ) ) {
							echo '<p>', sprintf( esc_html( $this->tgmpa->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'enigma-parallax' ) . '</a>' ), '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
